...
### Easy
<details><summary><b>What does DHCP stand for?</b></summary>DHCP stands for <b>Dynamic Host Configuration Protocol</b>.</details>
<details><summary><b>What is the purpose of a DHCP relay agent?</b></summary>A DHCP relay agent forwards DHCP broadcasts from clients to servers located on different networks. Since broadcasts cannot cross router boundaries, a <b>relay agent</b> is needed for clients to obtain IP information from a server on a different subnet.</details>
<details><summary><b>What command enables the DHCP service on a Cisco router?</b></summary>The command <code>service dhcp</code> enables the DHCP service on a Cisco router. However, this is usually not required as the service is enabled by <b>default</b>.</details>
<details><summary><b>How does a client indicate that it wants to use the address offered by a DHCP server?</b></summary>The client sends a broadcast <b>DHCPREQUEST</b> message to the server, indicating it will use the offered address and wants to lease it.</details>

### Medium
<details><summary><b>Describe the four-step process of DHCP address assignment.</b></summary>The DHCP address assignment process consists of four steps, often referred to as DORA:<br><b>Discover</b>: A client sends a broadcast DHCPDISCOVER message to locate DHCP servers.<br><b>Offer</b>: DHCP servers respond with DHCPOFFER messages containing an available IP address and other configuration details.<br><b>Request</b>: The client selects an offer and broadcasts a DHCPREQUEST message, requesting to lease the offered IP address.<br><b>Acknowledgment</b>: The chosen server confirms the lease with a DHCPACK message, including the lease duration and additional options.</details>
<details><summary><b>Can a router be configured as both a DHCP client and a DHCP server? Explain.</b></summary>Yes, a router can function as both a <b>DHCP client</b> and a DHCP server.<br>As a client, it can use the command <code>ip address dhcp</code> on an interface to obtain an IP address from another DHCP server.<br>As a server, it can be configured with an IP address pool, default gateway, DNS server, and other options to assign to clients. This involves defining excluded addresses, network address, subnet mask, and other parameters using the <code>ip dhcp excluded-address</code>, <code>ip dhcp pool</code>, <code>network</code>, <code>default-router</code>, and <code>dns-server</code> commands, respectively.</details>
<details><summary><b>What are the three dynamic address assignment options available for IPv6?</b></summary>IPv6 offers three dynamic addressing methods:<br><b>SLAAC</b> (Stateless Address Autoconfiguration): Devices automatically configure their IPv6 addresses using information from Router Advertisement (RA) messages.<br><b>Stateful DHCPv6</b>: A DHCPv6 server provides both address and additional configuration information to clients.<br><b>Stateless DHCPv6</b>: Devices obtain their addresses through SLAAC but rely on a DHCPv6 server for other configuration parameters.</details>
<details><summary><b>What is the function of the command "ip helper-address"?</b></summary>The <code>ip helper-address</code> command configures a router interface as a DHCP relay agent. It specifies the IP address of the DHCP server to which DHCP broadcasts should be forwarded. It's crucial to use the correct IP address and apply the command on the interface receiving client broadcasts.</details>

### Hard
<details><summary><b>Explain the concept of SLAAC and its limitations. How do stateful and stateless DHCPv6 address these limitations?</b></summary><b>SLAAC</b> (Stateless Address Autoconfiguration) allows devices to automatically generate their IPv6 address, prefix, and default gateway information from Router Advertisements (RAs) sent by routers. While this offers simplicity, its limitations include:<br><b>Limited Configuration Information</b>: SLAAC only provides basic addressing information. Devices might need additional parameters like DNS server addresses or domain names, which SLAAC cannot provide.<br>Stateful DHCPv6 overcomes these limitations by providing a centralized DHCPv6 server that assigns addresses and delivers other configuration parameters.<br>Stateless DHCPv6 combines the benefits of both approaches. Devices still utilize SLAAC for address assignment but obtain additional configuration parameters from a DHCPv6 server.</details>
<details><summary><b>Describe some common DHCP troubleshooting issues and provide solutions.</b></summary>Several issues can occur within a DHCP environment:<br><b>Router Not Forwarding Broadcasts</b>: If a client and server reside on different subnets, the router separating them needs to be configured as a DHCP relay agent using the <code>ip helper-address</code> command to forward broadcasts.<br><b>DHCP Pool Depletion</b>: If the DHCP server's pool of available IP addresses is exhausted, new client requests are rejected. Expanding the pool or reclaiming unused addresses can address this.<br><b>Misconfiguration</b>: Incorrectly configured DHCP server parameters, like an incorrect network address or subnet mask, can lead to assignment failures.<br><b>Duplicate IP Addresses</b>: Assigning a dynamic address that conflicts with a statically assigned IP will disrupt network connectivity.<br><b>Redundant Server Communication Issues</b>: If multiple DHCP servers are deployed for redundancy and their communication fails, they might assign overlapping addresses.</details>
<details><summary><b>Explain the purpose and function of the DHCPv6 relay agent.</b></summary>DHCPv6 clients use a link-local multicast address (FF02::1:2) when sending SOLICIT messages to discover DHCPv6 servers. Since link-local multicast addresses are confined to the local network, these messages cannot reach DHCPv6 servers located on different subnets.<br>A <b>DHCPv6 relay agent</b> is configured on the router interface connecting the client's network to the server's network. It captures the client's SOLICIT messages and forwards them as unicast messages to the DHCPv6 server's global unicast address. This allows clients to obtain IPv6 address information from servers located outside their local network.</details>
<details><summary><b>Discuss the DHCP troubleshooting commands mentioned in the source and their significance in diagnosing and resolving DHCP-related problems.</b></summary>The source highlights several useful commands for troubleshooting DHCP issues:<br><code>show ip dhcp conflict</code>: This command displays any detected IP address conflicts on the network. It lists the conflicting IP and detection method.<br><code>show ip dhcp binding</code>: This command provides a list of IP addresses that the DHCP server has leased to clients, along with their MAC addresses and lease duration.<br><code>debug ip dhcp server events</code>: This debug command displays real-time updates to the DHCP server database.<br><code>debug ip dhcp server packet</code>: This debug command captures and displays detailed information about DHCP packets exchanged between the server and clients.</details>
