...
<details><summary><b>What is the purpose of a DHCP relay agent?</b></summary>A DHCP relay agent forwards <b>DHCP broadcasts</b> from clients to servers located on different networks. Since broadcasts cannot cross router boundaries, a relay agent is needed for clients to obtain <b>IP information</b> from a server on a different subnet.</details>
<details><summary><b>What command enables the DHCP service on a Cisco router?</b></summary>The command <code>service dhcp</code> enables the DHCP service on a Cisco router. However, this is usually not required as the service is enabled by default.</details>
...
### Medium
<details><summary><b>Describe the four-step process of DHCP address assignment.</b></summary>The DHCP address assignment process consists of four steps, often referred to as <b>DORA</b>:<br><br>
<b>Discover</b>: A client sends a broadcast DHCPDISCOVER message to locate DHCP servers.<br>
<b>Offer</b>: DHCP servers respond with DHCPOFFER messages containing an available IP address and other configuration details.<br>
<b>Request</b>: The client selects an offer and broadcasts a DHCPREQUEST message, requesting to lease the offered IP address.<br>
<b>Acknowledgment</b>: The chosen server confirms the lease with a DHCPACK message, including the lease duration and additional options.</details>
<details><summary><b>Can a router be configured as both a DHCP client and a DHCP server? Explain.</b></summary>Yes, a router can function as both a DHCP client and a DHCP server.<br>
As a client, it can use the command <code>ip address dhcp</code> on an interface to obtain an IP address from another DHCP server.<br>
As a server, it can be configured with an IP address pool, default gateway, DNS server, and other options to assign to clients. This involves defining excluded addresses, network address, subnet mask, and other parameters using the <code>ip dhcp excluded-address</code>, <code>ip dhcp pool</code>, <code>network</code>, <code>default-router</code>, and <code>dns-server</code> commands.</details>
<details><summary><b>What are the three dynamic address assignment options available for IPv6?</b></summary>IPv6 offers three dynamic addressing methods:<br><br>
<b>SLAAC</b> (Stateless Address Autoconfiguration): Devices automatically configure their IPv6 addresses using information from Router Advertisement (RA) messages.<br>
<b>Stateful DHCPv6</b>: A DHCPv6 server provides both address and additional configuration information to clients.<br>
<b>Stateless DHCPv6</b>: Devices obtain their addresses through SLAAC but rely on a DHCPv6 server for other configuration parameters.</details>
<details><summary><b>What is the function of the command "ip helper-address"?</b></summary>The <code>ip helper-address</code> command configures a router interface as a <b>DHCP relay agent</b>. It specifies the IP address of the DHCP server to which DHCP broadcasts should be forwarded. It's crucial to use the correct IP address and apply the command on the interface receiving client broadcasts.</details>

### Hard
<details><summary><b>Explain the concept of SLAAC and its limitations. How do stateful and stateless DHCPv6 address these limitations?</b></summary>SLAAC (Stateless Address Autoconfiguration) allows devices to automatically generate their IPv6 address, prefix, and default gateway information from Router Advertisements (RAs) sent by routers. While this offers simplicity, its limitations include:<br><br>
<b>Limited Configuration Information</b>: SLAAC only provides basic addressing information. Devices might need additional parameters like DNS server addresses or NTP server information.<br>
<b>Lack of Centralized Management</b>: SLAAC lacks the centralized control provided by DHCP, making it difficult to track and manage address assignments across a network.<br><br>
<b>Stateful DHCPv6</b> overcomes these limitations by providing a centralized DHCPv6 server that assigns addresses and delivers other configuration parameters. <b>Stateless DHCPv6</b> combines both approaches, utilizing SLAAC for address assignment and a DHCPv6 server for additional configuration parameters.</details>
<details><summary><b>Describe some common DHCP troubleshooting issues and provide solutions.</b></summary>Several issues can occur within a DHCP environment:<br><br>
<b>Router Not Forwarding Broadcasts</b>: Configure the router as a DHCP relay agent using the <code>ip helper-address</code> command.<br>
<b>DHCP Pool Depletion</b>: Expand the pool or reclaim unused addresses.<br>
<b>Misconfiguration</b>: Verify and correct the server's configuration, such as the network address or subnet mask.<br>
<b>Duplicate IP Addresses</b>: Use address reservation or manage static assignments carefully.<br>
<b>Redundant Server Communication Issues</b>: Ensure proper communication between redundant DHCP servers.<br>
<b>Interface Mismatch</b>: Ensure the server interface belongs to the same subnet as the pool it manages.</details>
<details><summary><b>Explain the purpose and function of the DHCPv6 relay agent.</b></summary>DHCPv6 clients use a link-local multicast address (FF02::1:2) to discover servers. Since link-local multicast addresses are confined to the local network, a DHCPv6 relay agent is configured on the router to forward the client's SOLICIT messages to the DHCPv6 server's global unicast address. This allows clients to obtain IPv6 address information from servers located outside their local network. The <code>ipv6 dhcp relay destination</code> command configures the router interface as a relay agent, specifying the remote DHCPv6 server's address.</details>
<details><summary><b>Discuss the DHCP troubleshooting commands mentioned in the source and their significance in diagnosing and resolving DHCP-related problems.</b></summary>The source highlights several useful commands for troubleshooting DHCP issues:<br><br>
<code>show ip dhcp conflict</code>: Displays any detected IP address conflicts on the network.<br>
<code>show ip dhcp binding</code>: Lists IP addresses leased to clients along with their MAC addresses and lease duration.<br>
<code>debug ip dhcp server events</code>: Displays real-time updates of DHCP server events like address assignments, renewals, and releases.<br>
<code>debug ip dhcp server packet</code>: Captures and displays detailed information about DHCP packets exchanged between the server and clients.</details>
