...
<details><summary><b>How does a client indicate that it wants to use the address offered by a DHCP server?</b></summary>The client sends a broadcast <code>DHCPREQUEST</code> message to the server, indicating it will use the <b>offered</b> address and wants to lease it.</details><br>

### Medium
<details><summary><b>Describe the four-step process of DHCP address assignment.</b></summary>The DHCP address assignment process consists of four steps, often referred to as <b>DORA</b>:<br>Discover: A client sends a broadcast <code>DHCPDISCOVER</code> message to locate DHCP servers.<br>Offer: DHCP servers respond with <code>DHCPOFFER</code> messages containing an available IP address and other configuration details.<br>Request: The client selects an offer and broadcasts a <code>DHCPREQUEST</code> message, requesting to lease the offered IP address.<br>Acknowledgment: The chosen server confirms the lease with a <code>DHCPACK</code> message, including the lease duration and additional options.</details><br>
<details><summary><b>Can a router be configured as both a DHCP client and a DHCP server? Explain.</b></summary>Yes, a router can function as both a DHCP <b>client</b> and a DHCP <b>server</b>. As a client, it can use the command <code>ip address dhcp</code> on an interface to obtain an IP address from another DHCP server. As a server, it can be configured with an IP address pool, default gateway, DNS server, and other options to assign to clients. This involves defining excluded addresses, network address, subnet mask, and other parameters using the <code>ip dhcp excluded-address</code>, <code>ip dhcp pool</code>, <code>network</code>, <code>default-router</code>, and <code>dns-server</code> commands, respectively.</details><br>
<details><summary><b>What are the three dynamic address assignment options available for IPv6?</b></summary>IPv6 offers three dynamic addressing methods:<br>SLAAC (<b>Stateless</b> Address Autoconfiguration): Devices automatically configure their IPv6 addresses using information from Router Advertisement (RA) messages.<br>Stateful DHCPv6: A DHCPv6 server provides both address and additional configuration information to clients.<br>Stateless DHCPv6: Devices obtain their addresses through SLAAC but rely on a DHCPv6 server for other configuration parameters.</details><br>
